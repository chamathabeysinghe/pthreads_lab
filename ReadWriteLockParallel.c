...
int thread_count;
pthread_rwlock_t linkedListMutex;
...
int Member(node head,int value){
    ...
    pthread_rwlock_rdlock(&linkedListMutex);
    ...
    pthread_rwlock_unlock(&linkedListMutex);
    return 0;
}

node Insert(node head,int value){
    pthread_rwlock_wrlock(&linkedListMutex);
    node temp,p;
    temp = createNode();
    temp->value = value;
    if(head == NULL){
...
int main(int argc,char* argv[]){
    ...
    pthread_rwlock_init(&linkedListMutex, NULL);
